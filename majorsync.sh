...
establish_ssh_tunnel(){
  ...
  ssh -t -t -L 62000:localhost:2379 "core@$ip_address" &> /dev/null
}

kill_ssh_tunnel_job() {
  local parent_pid="$1"
  pkill -P "$parent_pid" -f 'ssh -t -t -L 62000:localhost:2379'
}
...
do_etcd_sync(){
  ...
  env \
    ...
    etcdsync \
      --etcd-uri http://localhost:62000 \
      --local-path "$HOME/Projects/Octoblu/the-stack-env-production/major/etcd" \
...
wait_for_tunnel() {
  ...
  while [ "$tunnel_open" != "0" ]; do
    echo -n "."
    curl http://localhost:62000 &> /dev/null
    tunnel_open="$?"
    sleep 0.25
  done
  echo ""
...
assert_port_free() {
  ...
  local exit_code="$?"
  if [ "$exit_code" == "0" ]; then
    echo "Port 62000 seems to be in use, cowardly refusing to do anything"
    exit 1
  fi
}
...
usage(){
  echo "USAGE: majorsync [options] <dump/gd/gload/l/printfs/pf/printetcd/pe> <project-name>"
  echo ""
  echo "example: majorsync load governator-service"
